...
async function runJuno(u, messages, mode, device) {
  ...
  while (guard++ < 22) {
    const d = await claude({ model: MODEL, max_tokens: 8000, system: SYSTEM(u, mode, device), tools, messages });
...
async function runAgent(u, agent) {
  ...
  let result;
  try { const { text } = await runJuno(u, messages, ''); result = text; logAct(u, 'agent', `🤖 ${agent.name}: ${text.slice(0, 100)}`); }
  catch (e) { result = 'Error: ' + e.message; }
  agent.lastResult = result; agent.lastResultAt = new Date().toISOString();
  agent.runs = agent.runs || []; agent.runs.unshift({ at: new Date().toISOString(), result }); agent.runs = agent.runs.slice(0, 25);
  save();
}
function agentDue(a) {
